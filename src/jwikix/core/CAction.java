...
import jwiki.core.WAction;
import jwiki.core.Wiki;
import jwiki.util.FL;
import jwiki.util.Tuple;
import jwikix.util.PTask;
...
public final class CAction
{
	...
	{
		return PTask.execute(FL.toAL(titles.stream().map(t -> new Tuple<>(t, () -> wiki.delete(t, reason)))));
	}
	...
	{
		return PTask.execute(FL.toAL(titles.stream().map(t -> new Tuple<>(t, () -> wiki.addText(t, add, reason, append)))));
	}
